  ...
  dynamic "rule_group" {
    ...
    content {
      ...
      dynamic "rule_variables" {
        for_each = lookup(rule_group, "rule_variables", {})
        content {
          # Set of configuration blocks that define IP address information
          dynamic "ip_sets" {
            for_each = lookup(rule_variables, "ip_sets", [])
            content {
          ...
          dynamic "port_sets" {
            for_each = lookup(rule_variables, "port_sets", [])
            content {
      ...
      rules_source {
        rules_string = lookup(lookup(rule_group, "rules_source", {}), "rules_string_file_path", null) != null ? file(rule_group.rules_source.rules_string_file_path) : null

        dynamic "rules_source_list" {
          for_each = lookup(lookup(rule_group, "rules_source", {}), "rules_source_list", {})
          content {
            # String value to specify whether domains in the target list are allowed or denied access. Valid values: ALLOWLIST, DENYLIST
            generated_rules_type = rules_source_list.generated_rules_type
            # Set of types of domain specifications that are provided in the targets argument. Valid values: HTTP_HOST, TLS_SNI
            target_types = rules_source_list.target_types
            # Set of domains that you want to inspect for in your traffic flows
            targets = rules_source_list.targets
          }
        }
        dynamic "stateful_rule" {
          for_each = lookup(lookup(rule_group, "rules_source", {}), "stateful_rule", {})
          content {
        ...
        dynamic "stateless_rules_and_custom_actions" {
          for_each = lookup(lookup(rule_group, "rules_source", {}), "stateless_rules_and_custom_actions", {})
          content {
            dynamic "custom_action" {
              for_each = lookup(stateless_rules_and_custom_actions, "custom_action", {})
              content {
                action_name = custom_action.action_name
                action_definition {
                  publish_metric_action {
                    dynamic "dimension" {
                      for_each = custom_action.dimensions
                      content {
            ...
            dynamic "stateless_rule" {
              for_each = lookup(stateless_rules_and_custom_actions, "stateless_rule", {})
              content {
                priority = stateless_rule.priority
                rule_definition {
                  actions = stateless_rule.rule_definition.actions
                  match_attributes {
                    protocols = lookup(stateless_rule.rule_definition.match_attributes, "protocols", [])
                    dynamic "destination" {
                      for_each = lookup(stateless_rule.rule_definition.match_attributes, "destination", [])
                      content {
                        address_definition = destination.value
                      }
                    }
                    dynamic "destination_port" {
                      for_each = lookup(stateless_rule.rule_definition.match_attributes, "destination_port", [])
                      content {
                    ...
                    dynamic "source" {
                      for_each = lookup(stateless_rule.rule_definition.match_attributes, "source", [])
                      content {
                        address_definition = source.value
                      }
                    }
                    dynamic "source_port" {
                      for_each = lookup(stateless_rule.rule_definition.match_attributes, "source_port", [])
                      content {
                    ...
                    dynamic "tcp_flag" {
                      for_each = lookup(stateless_rule.rule_definition.match_attributes, "tcp_flag", [])
                      content {
